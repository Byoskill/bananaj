...
public class Campaign extends MailchimpObject {
     ...
     */
    public Campaign(String id, MailChimpList mailChimpList, CampaignType campaign_type, CampaignStatus campaign_status, CampaignSettings campaignSettings,
    ...
    public Campaign(MailChimpConnection connection, JSONObject campaign) throws Exception {
        ...
        if (recipients.has("list_id")) {
            this.mailChimpListId = recipients.getString("list_id");
    ...
    public void overwriteSettings(CampaignSettings campaignSettings) throws CampaignSettingsException, Exception {
        ...
        this.getConnection().do_Patch(new URL(this.getConnection().getCampaignendpoint() + "/" + this.getId()), updatedCampaign.toString(),
    ...
    public Report getReport() throws Exception {
        ...
        Click clicksObject = new Click(clicks.getInt("clicks_total"), clicks.getInt("unique_clicks"), clicks.getInt("unique_subscriber_clicks"),
                clicks.getDouble("click_rate"), DateConverter.getInstance().createDateFromISO8601(clicks.getString("last_click")));
        ...
        FacebookLikes facebookObject = new FacebookLikes(facebook_likes.getInt("recipient_likes"), facebook_likes.getInt("unique_likes"),
                facebook_likes.getInt("facebook_likes"));
        IndustryStats industryStatsObject = new IndustryStats(industry_stats.getString("type"), industry_stats.getDouble("open_rate"),
                industry_stats.getDouble("click_rate"), industry_stats.getDouble("bounce_rate"), industry_stats.getDouble("unopen_rate"),
                industry_stats.getDouble("unsub_rate"), industry_stats.getDouble("abuse_rate"));
        ReportListStats reportListStatsObject = new ReportListStats(report_list_stats.getDouble("sub_rate"), report_list_stats.getDouble("unsub_rate"),
                report_list_stats.getDouble("open_rate"), report_list_stats.getDouble("click_rate"));


        return new Report(report.getString("id"), report.getString("campaign_title"), report.getInt("emails_sent"), report.getInt("abuse_reports"),
                report.getInt("unsubscribed"), DateConverter.getInstance().createDateFromISO8601(report.getString("send_time")), bouncesObject, forwardsObject
                , clicksObject, opensObject, facebookObject, industryStatsObject, reportListStatsObject, report);
    }
     ...
     */
    public MailChimpList getMailChimpList() throws Exception {

        if (mailChimpList == null) {
            mailChimpList = connection.getList(mailChimpListId);
        }
    ...
    public void changeHTMLContent(String htmlContent) throws Exception{
        ...
        final String html = jsonContent.has("html") ? jsonContent.getString("html") : null;
        this.content = new CampaignContent(plain_text, html, this);
    ...
    public void setContent() throws Exception {
        JSONObject jsonContentFromCall = new JSONObject(getConnection().do_Get(new URL(connection.getCampaignendpoint() + "/" + this.getId() + "/content"),
    ...
    public void pause() throws Exception {
        final String source = getConnection().do_Post(new URL(connection.getCampaignendpoint() + "/" + this.getId() + "/actions/pause"),
                connection.getApikey());
        if (source != null) {
